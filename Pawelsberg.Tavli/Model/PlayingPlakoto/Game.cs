...
public record Game : GameBase
{
    ...
    internal PlayerColour? GetCurrentTurnPlayer()
    {
        PlayerColour? currentTurnPlayer =
            State == GameState.PlayerWonRollForOrder
            ? StatePlayer.Value
            : State == GameState.PlayerMovedOrTiredOrBearedOff
    ...
    public IEnumerable<PlayerColour> GetPlayersRolling()
    {
        switch (State)
        {
            ...
            case GameState.PlayerMovedOrTiredOrBearedOff:
    ...
    public IEnumerable<TurnPlay> GetAllTurnPlays(TurnRoll turnRoll, bool removeRedundantPlays = true)
    {
        ...
        if (State != GameState.PlayerWonRollForOrder && State != GameState.PlayerMovedOrTiredOrBearedOff)
    ...
    public int CheckersCoveredBy(PlayerColour playerColour)
    {
        PlayerColour oponentColour = playerColour.GetNext();
        return Board
            .Points
            .Where(p => p.Checkers.FirstOrDefault()?.Colour == oponentColour)
            .Count(p => p.Checkers.Skip(1).FirstOrDefault()?.Colour == playerColour);
    }
    public int CheckersProneToCovering(PlayerColour player)
    {
        int playerStartingPosition = player == PlayerColour.White ? 23 : 0;
        int playerDirection = player == PlayerColour.White ? -1 : 1;

        List<(Point p, int wi, int bi)> pointWiBi = Board
            .Points
            .Select((p, i) => (p, playerStartingPosition - i, i))
            .ToList();
            ...
            if (checkerExistsOnSourcePosition)
            {
                ...
                { // move it if possible
                    ...
                    { // if destination available
                        ...
                        bool uncovering = Board.Points[sourcePosition].Checkers.Count == 2 && Board.Points[sourcePosition].Checkers[0].Colour != currentPlayer;
    ...
    public static Game ParseFromStringRepresentation(string text)
    {
        ...
        Game result = new Game(board, state, playerColour, bearedOffCheckers);
        // TODO - check pips
