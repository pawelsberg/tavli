...
#pragma warning restore CS0252 // Possible unintended reference comparison; left hand side needs cast
    ...
    public int GetHashCode(Game portesGame)
    {
        return portesGame.Board.GenerateHashCode()
            ^ portesGame.State.GetHashCode()
            ^ (int)(portesGame.StatePlayer ?? 0)
            ^ portesGame.BearedOffCheckers[PlayerColour.White].Aggregate(0, (acc, c) => acc ^ c.GetHashCode())
            ^ portesGame.BearedOffCheckers[PlayerColour.Black].Aggregate(0, (acc, c) => acc ^ c.GetHashCode())
        ;
    }
}
